...
from transition_matrix.makeplots.argo_data import Argo
noise_factor = 0.05
...
class CovBase(BaseMat):
	...
	def __init__(self,arg1,shape=None,total_list=None,lat_spacing=None,lon_spacing=None,l=None,traj_file_type=None):
		self.l = l
		super(CovBase,self).__init__(arg1, shape=shape,total_list=total_list,lat_spacing=lat_spacing,lon_spacing=lon_spacing,traj_file_type=traj_file_type)

class InverseInstance(CovBase):
	def __init__(self,arg1,shape=None,total_list=None,lat_spacing=None,lon_spacing=None,l=None,variable_list=None,traj_file_type='covariance'):
		super(InverseInstance,self).__init__(arg1, shape=shape,total_list=total_list,lat_spacing=lat_spacing,lon_spacing=lon_spacing,l=l,traj_file_type=traj_file_type)
		self.variable_list = variable_list

	def get_cov(self,row_var,col_var):
		row_idx = self.variable_list.index(row_var)
		col_idx = self.variable_list.index(col_var)
		split_array = np.split(np.split(self.todense(),4)[row_idx],4,axis=1)[col_idx]
	...
	@staticmethod
	def load_from_type(lat_spacing=None,lon_spacing=None,l=None,traj_type='covariance'):
...
class CovInstance(CovBase):
	def __init__(self,arg1,shape=None,total_list=None,lat_spacing=None,lon_spacing=None,l=None,var1=None,var2=None,traj_file_type='covariance_instance'):
		self.var1 = var1
		self.var2 = var2
		super(CovInstance,self).__init__(arg1, shape=shape,total_list=total_list,lat_spacing=lat_spacing,lon_spacing=lon_spacing,l=l,traj_file_type=traj_file_type)
	...
	def plot_variance(self):
		...
		vmax_holder = min(plottable_mean+2*plottable_std,self.diagonal().max())
		vmin_holder = max(plottable_mean-2*plottable_std,self.diagonal().min())
		if self.var1==self.var2:
			vmax_holder = min(plottable_mean+2*plottable_std,self.diagonal().max())
			vmin_holder = max(plottable_mean-2*plottable_std,self.diagonal().min())
			vmin_holder = max(0,vmin_holder)
		XX,YY,m = basemap_setup(lat_grid,lon_grid,'')
		m.pcolormesh(XX,YY,plottable,vmax=vmax_holder,vmin=vmin_holder)
		plt.colorbar(label=var1+'-'+var2+' variance')
		plt.savefig(base+self.var1+'-'+self.var2)
		plt.close()

class HInstance(scipy.sparse.csc_matrix):
	def __init__(self, arg1, shape=None):
		super(HInstance,self).__init__(arg1, shape=shape)

	@staticmethod
	def randomly_generate(number,total_list=None,variable_list=None,degree_bins=None):
		if type(total_list)==np.ndarray:
			total_list = total_list.tolist()
		lat,lon = zip(*random.sample(total_list,number))
		locations = []
	...
	def generate_from_locations(locations,total_list=None,variable_list=None,degree_bins=None):
		...
		for loc in locations:
			...
			try:
				tl_idx = total_list.index([rounded_lat,rounded_lon])
...
def plot_array_variance_constrained():
	base = ROOT_DIR+'/plots/cm2p6_covariance/'
	transmat = TransMat.load_from_type(2,2,60)
	...
	cov = InverseInstance.load_from_type(2,2,300)
	hinstance = HInstance.generate_from_existing_array([argo,soccom],transmat = transmat,
		new_total_list=cov.total_list,variable_list=cov.variable_list,degree_bins=cov.degree_bins).T
	output_mask = np.array(hinstance.sum(axis=0)>0).ravel()
	noise = scipy.sparse.diags([cov.diagonal().mean()*noise_factor]*hinstance.shape[0])
	denom = hinstance.dot(cov).dot(hinstance.T)+noise
	inv_denom = scipy.sparse.linalg.inv(denom)
...
def targeted_array_plots():
	...
	invinstance = InverseInstance.load_from_type(2,2,300)
	(bins_lat,bins_lon)=BaseMat.bins_generator(invinstance.degree_bins)
	...
	for num in num_list:
		...
		for k in range(10):
			print(k)
			hinstance = HInstance.randomly_generate(num,total_list=invinstance.total_list,variable_list=[covinstance.var1],degree_bins=invinstance.degree_bins)
			p_hat = calculate_phat(hinstance,covinstance)
