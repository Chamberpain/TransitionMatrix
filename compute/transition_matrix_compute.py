...
class base_info():
    def __init__(self,degree_bin_lat,degree_bin_lon,date_span_limit,traj_file_type):
        ...
        if self.traj_file_type=='Crete':
            self.base_file = os.getenv("HOME")+'/iCloud/Data/Processed/transition_matrix/crete/'  #this is a total hack to easily change the code to run sose particles
            # self.base_file = os.getenv("HOME")+'/transition_matrix/'
...
################## plotting routines ###########################
    ...
    def profile_density_plot(self): #plot the density of profiles
        ...
        m = Basemap(projection='cea',fix_aspect=False)
...
class transition():
    def __init__(self,information,traj,float_type,season):
        self.info = information
...
#this algorithm has problems with low data density because the frame does not drop na values for the self.end_bin_string
    ...
    def matrix_recalc(self,index_list,old_total_list,transition_matrix,number_matrix,noise=True):
        ...
        number_matrix = self.matrix_coordinate_change(number_matrix,old_total_list,self.transition.list)
        if noise:
    ...
    def matrix_plot_setup(self):
        ...
        elif self.info.traj_file_type == 'Argo':
        ...
        elif self.info.traj_file_type == 'Crete':
...
def transition_class_loader(degree_bin_lat=2,degree_bin_lon=3,date_span_limit=100,traj_file_type='Argo'):
