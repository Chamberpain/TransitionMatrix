...
for span_limit in [60,120]:
	traj_class = argo_traj_data(degree_bins=2,date_span_limit=span_limit)
	for column in range(traj_class.transition_matrix.shape[0]):
		...
		lat,lon = zip(*[tuple(traj_class.total_list[x]) for x in traj_class.transition_matrix[:,0].indices.tolist()])
