...
class CovArray(object):
	def __init__(self,*args,lat_sep=2,lon_sep=2,l=300,depth_idx=0,**kwargs):
		...
		self.lats,self.lons,self.truth_array,self.lat_grid,self.lon_grid,self.index_list \
		 = self.lats_lons(lat_sep=lat_sep,lon_sep=lon_sep)
	...
	def save(self):
		...
		if not os.path.isdir(filedir):
			os.mkdir(filedir)
		mat_obj = InverseInstance(self.global_cov,shape = self.global_cov.shape,total_list = self.index_list,\
			lat_spacing = self.lat_sep, lon_spacing = self.lon_sep,variable_list=self.variable_list,l=5*self.l,traj_file_type=self.label+'_global_covariance_'+str(self.depth_idx))
		mat_obj.save()
		filename = InverseInstance.make_filename(self.label+'_submeso_covariance_'+str(self.depth_idx),[self.lat_sep,self.lon_sep],l=self.l)
		filedir = os.path.dirname(filename)
		if not os.path.isdir(filedir):
			os.mkdir(filedir)
		mat_obj = InverseInstance(self.submeso_cov,shape = self.submeso_cov.shape,total_list = self.index_list,\
			lat_spacing = self.lat_sep, lon_spacing = self.lon_sep,variable_list=self.variable_list,l=self.l,traj_file_type=self.label+'_submeso_covariance_'+str(self.depth_idx))
		mat_obj.save()
	...
	def get_dist(self):
		def calculate_distance():
			...
			lats,lons,truth_list,lat_grid,lon_grid,index_list = self.lats_lons(lat_sep=self.lat_sep,lon_sep=self.lon_sep)
		...
		filename = self.data_directory+'distance_lat_'+str(self.lat_sep)+'_lon_'+str(self.lon_sep)
		try:
...
class CovCM2p6(CovArray):
	data_directory = ROOT_DIR + '/data/cm2p6'
	variable_list = ['o2','dic','temp','salt']
	label = 'cm2p6'
	...
	@staticmethod
	def lats_lons(lat_sep=None,lon_sep=None):
...
class CovCM4(CovArray):
	data_directory = ROOT_DIR + '/../../data/cm4'
	print('data_directory is '+data_directory)
	chl_depth_idx = 10
	label = 'cm4'
	def __init__(self,*args,**kwargs):
		super().__init__(*args,**kwargs)
		if self.depth_idx<self.chl_depth_idx:
			self.variable_list = ['thetao','so','ph','chl','o2']
		else:
			self.variable_list = ['thetao','so','ph','o2']
	...
	def make_matrix(self):
		...
		assert check_symmetric(out_mat)
		return out_mat 

	@staticmethod
	def lats_lons(lat_sep=2,lon_sep=2):
		max_depth_lev = 25  #this corresponds to 2062.5 meters 
		files,var = CovCM4.get_filenames()[0]
		file = files[0]
		dh = Dataset(file)
		temp = dh[var][:,max_depth_lev,:,:]
		depth_mask = temp.mask[0]
		X,Y = np.meshgrid(np.floor(dh['lon'][:].data),np.floor(dh['lat'][:]).data)
		subsample_mask = ~((X%lon_sep==0)&(Y%lat_sep==0))
		total_mask = depth_mask|subsample_mask
		total_mask = ~total_mask
		X = X[total_mask]
		Y = Y[total_mask]
		lat_grid = np.arange(-90,91,lat_sep)
		lon_grid = np.arange(-180,180,lon_sep)
		index_list = [list(x) for x in list(zip(X,Y))]
		return (Y,X,total_mask,lat_grid,lon_grid,index_list)
